...
NCRMP_DRM_calculate_species_richness_diversity <- function(project, region){
  ...
  #Old: for(k in 1:length(tmp))assign(names(tmp)[k], tmp[[k]])
  list2env(tmp, envir = environment())


  #### Function to format geospatial data ####
  format_geospatial_data <- function(data){
    data <- data %>%
      dplyr::mutate(LAT_DEGREES = sprintf("%0.4f", LAT_DEGREES), # Format latitude to 4 decimals
                    LON_DEGREES = sprintf("%0.4f", LON_DEGREES),  # Format longitude to 4 decimals
                    PROT = as.factor(PROT))
  }


  # Apply formatting to the 1-stage data
  dat_1stage <- format_geospatial_data(dat_1stage)

  #If tmp has more than one element, apply formatting to the 2-stage datas
  if(length(tmp) > 1){
  ...
  dat <- switch(region,
                ...
                "PRICO" = dat_1stage,
                "GOM" = dat_1stage,
                NULL #Defult case if no region
  )

  # Clean up species names
  # this get used later for diversity
  clean_species_names <- function(data){
    data <- data %>%
  ...
  #apply clean_species_names function to dat and dat_1stage
  dat <- clean_species_names(dat)
  dat_1stage <- clean_species_names(dat_1stage)

  if(project == "NCRMP_DRM" ||
     project == "NCRMP" && region == "SEFCRI" ||
     project == "NCRMP" && region == "Tortugas") {

    dat_2stage <- clean_species_names(dat_2stage)
  }
  ...
  if(project == "NCRMP_DRM" ||
    ...
    species_list <- dplyr::bind_rows(species_1stage, species_2stage) %>%
      unique()
