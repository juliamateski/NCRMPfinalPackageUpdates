...
NCRMP_DRM_calculate_mean_colony_size <- function(project = "NULL", region, species_filter = "NULL"){
  ...
  clean_data <- function(data) {
    new <- data %>%
      dplyr::mutate(
        total_mort = OLD_MORT + RECENT_MORT,
        PROT = as.factor(PROT)
      ) %>%
      dplyr::filter(
  ...
  summarize_function <- function(data) {
    data <- data%>%
      dplyr::summarise(
        avg_cm2 = mean(size_2d, na.rm = TRUE),
  ...
  summarize_function_2 <- function(data) {
    data <- data%>%
...
r <- NCRMP_DRM_calculate_mean_colony_size(region = "SEFCRI", project = "NCRMP_DRM")
print(r)
