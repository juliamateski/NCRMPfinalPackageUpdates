...
load_NCRMP_DRM_demo_data <- function(project = "NULL", region, species_filter = "NULL"){
  ...
  mutate_undaria <- function(data) {
    data %>% dplyr::mutate(SPECIES_NAME = case_when(SPECIES_NAME == "Undaria spp" ~ "Agaricia spp", TRUE ~ SPECIES_NAME))
  }
  ...
  if(region == "GOM"){
    ...
    dat_1stage <- dplyr::bind_rows(datasets) %>%
      dplyr::mutate(SURVEY = "NCRMP", STRAT = "FGBNMS", REGION = "GOM")
