...
updated_NCRMP_DRM_calculate_mortality <- function(project, region, species_filter = "NULL"){

  tmp <- load_NCRMP_DRM_demo_data(project = project, region = region, species_filter = species_filter)

  #unlack list
  #(k in 1:length(tmp))assign(names(tmp)[k], tmp[[k]])
  ...
  calculate_mortality <- function(data, mortality_col, type) {
    data %>%
      dplyr::summarise(avsitemort = mean({{mortality_col}}, na.rm = TRUE), .groups = "keep") %>%
      dplyr::mutate(MORT_TYPE = type) %>%
      dplyr::ungroup()
  }
  ...
  if(project == "NCRMP_DRM" ||
    ...
    dat1_1stage <- dat_1stage %>%
      ...
      calculate_mortality(mortality_col = OLD_MORT, type = "Old")
    ...
    dat1_2stage <- dat_2stage %>%
      ...
      dplyr::group_by(REGION, SURVEY, YEAR, SUB_REGION_NAME, PRIMARY_SAMPLE_UNIT, LAT_DEGREES, LON_DEGREES, STRAT, HABITAT_CD, PROT) %>%
      calculate_mortality(mortality_col = transect_mort, type = "Old")
  ...
  if(project == "NCRMP_DRM" ||
    ...
    dat1_1stage <- dat_1stage %>%
      ...
      calculate_mortality(mortality_col = RECENT_MORT, type = "Recent")
    ...
    dat1_2stage <- dat_2stage %>%
      ...
      calculate_mortality(mortality_col = transect_mort, type = "Recent")
